-- Atomic PO/SO status recomputation to eliminate race conditions
-- when concurrent receipts/shipments update the parent order status.
...
GRANT EXECUTE ON FUNCTION update_po_status_from_items(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION update_so_status_from_items(UUID, UUID) TO authenticated;
