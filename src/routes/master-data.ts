...
masterData.get('/products', async (c) => {
  ...
  let query = db
    .from('products')
    .select('id, name, code, description, type, uom, status, cost_price, sale_price, category:product_categories(id,name)', { count: 'exact' })
...
masterData.post('/products', async (c) => {
  ...
  const ALLOWED: Record<string, boolean> = {
    ...
    uom: true, type: true, is_lot_controlled: true, is_serial_controlled: true,
...
const productCategoriesConfig: CrudConfig = {
  ...
  detailSelect: 'id, code, name, parent_id, level, is_active, organization_id, created_at, updated_at, parent:product_categories(id,name,code)',
