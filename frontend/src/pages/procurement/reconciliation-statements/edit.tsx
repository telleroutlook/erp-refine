...
export const ReconciliationStatementEdit: React.FC = () => {
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title={pt('reconciliation_statements', 'edit')}>
      ...
      <EditableItemTable resource="reconciliation-lines" parentResource="reconciliation-statements" parentId={record?.id} parentFk="reconciliation_statement_id" items={record?.items ?? []} columns={itemColumns} title={t('sections.reconciliationLines')} />
    </Edit>
  );
};
