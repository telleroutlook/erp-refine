...
export const SupplierQuotationEdit: React.FC = () => {
  ...
  const lineColumns: ColumnConfig[] = [
    ...
    { dataIndex: 'total_price', title: '合计', width: 120, align: 'right', render: (v: any) => <AmountDisplay value={v} currency={record?.currency} /> },
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title="编辑供应商报价">
      <Form {...formProps} layout="vertical">
        <Row gutter={16}>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="报价单号" name="quotation_number"><Input disabled /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="状态" name="status"><Select options={QUOTATION_STATUS_OPTIONS} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="货币" name="currency"><Select options={CURRENCY_OPTIONS} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="有效期" name="validity_date" {...dateFormItemProps}><DatePicker style={FULL_WIDTH} /></Form.Item>
          </Col>
          <Col span={24}>
      ...
      <EditableItemTable resource="supplier-quotation-lines" parentResource="supplier-quotations" parentId={record?.id} parentFk="supplier_quotation_id" items={record?.lines ?? []} columns={lineColumns} title="报价行" />
    </Edit>
  );
};
