...
export const AdvanceShipmentNoticeEdit: React.FC = () => {
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title="编辑ASN">
      <Form {...formProps} layout="vertical">
        <Row gutter={16}>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="ASN编号" name="asn_no"><Input disabled /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="状态" name="status"><Select options={ASN_STATUS_OPTIONS} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="供应商" name="supplier_id"><Select {...supplierProps} showSearch /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="仓库" name="warehouse_id"><Select {...warehouseProps} showSearch /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="预计到货日" name="expected_date" {...dateFormItemProps}><DatePicker style={FULL_WIDTH} /></Form.Item>
          </Col>
          <Col span={24}>
      ...
      <EditableItemTable resource="asn-lines" parentResource="advance-shipment-notices" parentId={record?.id} parentFk="asn_id" items={record?.items ?? []} columns={itemColumns} title="ASN行项" />
    </Edit>
  );
};
