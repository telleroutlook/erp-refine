...
export const PurchaseRequisitionEdit: React.FC = () => {
  ...
  const itemColumns: ColumnConfig[] = [
    ...
    { dataIndex: 'amount', title: '行合计', width: 120, align: 'right', render: (v: any) => <AmountDisplay value={v} /> },
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title="编辑采购申请">
      <Form {...formProps} layout="vertical">
        <Row gutter={16}>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="申请单号" name="requisition_number"><Input disabled /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="状态" name="status"><Select options={REQUISITION_STATUS_OPTIONS} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="申请日期" name="request_date" {...dateFormItemProps}><DatePicker style={FULL_WIDTH} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="需求日期" name="required_date" {...dateFormItemProps}><DatePicker style={FULL_WIDTH} /></Form.Item>
          </Col>
          <Col span={24}>
      ...
      <EditableItemTable
        ...
        parentResource="purchase-requisitions"
        parentId={record?.id}
        parentFk="purchase_requisition_id"
        items={record?.lines ?? []}
        columns={itemColumns}
        title="申请行"
      />
