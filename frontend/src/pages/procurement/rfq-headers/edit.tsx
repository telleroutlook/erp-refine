...
export const RfqHeaderEdit: React.FC = () => {
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title="编辑询价单">
      <Form {...formProps} layout="vertical">
        <Row gutter={16}>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="询价单号" name="rfq_number"><Input disabled /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="状态" name="status"><Select options={RFQ_STATUS_OPTIONS} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="截止日期" name="due_date" {...dateFormItemProps}><DatePicker style={FULL_WIDTH} /></Form.Item>
          </Col>
          <Col span={24}>
      ...
      <EditableItemTable resource="rfq-lines" parentResource="rfq-headers" parentId={record?.id} parentFk="rfq_header_id" items={record?.lines ?? []} columns={lineColumns} title="询价行" />
    </Edit>
  );
};
