...
export const PurchaseReceiptEdit: React.FC = () => {
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title="编辑采购收货单">
      <Form {...formProps} layout="vertical">
        <Row gutter={16}>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="收货单号" name="receipt_number"><Input disabled /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="状态" name="status"><Select options={RECEIPT_STATUS_OPTIONS} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="收货日期" name="receipt_date" {...dateFormItemProps}><DatePicker style={FULL_WIDTH} /></Form.Item>
          </Col>
          <Col span={24}>
      ...
      <EditableItemTable
        resource="purchase-receipt-items"
        parentResource="purchase-receipts"
        parentId={record?.id}
        parentFk="purchase_receipt_id"
        items={record?.items ?? []}
        columns={itemColumns}
        title="收货行"
      />
