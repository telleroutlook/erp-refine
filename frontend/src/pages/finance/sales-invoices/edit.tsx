...
export const SalesInvoiceEdit: React.FC = () => {
  ...
  const itemColumns: ColumnConfig[] = [
    { dataIndex: 'product_id', title: fl('sales_invoices', 'product_id'), editable: true, inputType: 'select', selectOptions: productOptions, render: (_: any, r: any) => r?.product?.name },
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title={pt('sales_invoices', 'edit')}>
      ...
      <EditableItemTable resource="sales-invoice-items" parentResource="sales-invoices" parentId={record?.id} parentFk="sales_invoice_id" items={record?.items ?? []} columns={itemColumns} title={t('sections.invoiceLines')} productsMap={productsMap} priceField="sale_price" />
    </Edit>
  );
};
