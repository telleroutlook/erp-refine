import React from 'react';
import { useShow } from '@refinedev/core';
import { Show, DateField } from '@refinedev/antd';
import { Descriptions } from 'antd';
import { useTranslation } from 'react-i18next';
import { StatusTag } from '../../../components/shared/StatusTag';
import { AmountDisplay } from '../../../components/shared/AmountDisplay';

export const PaymentRecordShow: React.FC = () => {
  const { t } = useTranslation();
  const { queryResult } = useShow({ resource: 'payment-records' });
  const record = queryResult.data?.data as any;

  return (
    <Show title={`付款记录 ${record?.payment_number ?? ''}`} isLoading={queryResult.isLoading}>
      <Descriptions bordered size="small" column={2}>
        <Descriptions.Item label="付款编号">{record?.payment_number}</Descriptions.Item>
        <Descriptions.Item label="付款日期">
          <DateField value={record?.payment_date} format="YYYY-MM-DD" />
        </Descriptions.Item>
        <Descriptions.Item label="付款类型">{record?.payment_type}</Descriptions.Item>
        <Descriptions.Item label="付款方式">{record?.payment_method}</Descriptions.Item>
        <Descriptions.Item label={t('common.amount')}>
          <AmountDisplay value={record?.amount} currency={record?.currency} />
        </Descriptions.Item>
        <Descriptions.Item label="对象类型">{record?.partner_type}</Descriptions.Item>
        <Descriptions.Item label="对象ID">{record?.partner_id}</Descriptions.Item>
        <Descriptions.Item label="关联类型">{record?.reference_type}</Descriptions.Item>
        <Descriptions.Item label="关联ID">{record?.reference_id}</Descriptions.Item>
        <Descriptions.Item label={t('common.status')}>
          <StatusTag status={record?.status} />
        </Descriptions.Item>
        {record?.notes && <Descriptions.Item label={t('common.notes')} span={2}>{record.notes}</Descriptions.Item>}
      </Descriptions>
    </Show>
  );
};
