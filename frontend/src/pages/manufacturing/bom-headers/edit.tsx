...
export const BomHeaderEdit: React.FC = () => {
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title="编辑物料清单">
      <Form {...formProps} layout="vertical">
        <Row gutter={16}>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="BOM编号" name="bom_number"><Input disabled /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="产品" name="product_id" rules={[{ required: true }]}><Select options={productOptions} showSearch optionFilterProp="label" /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="基准数量" name="quantity" rules={[{ required: true }]}><InputNumber style={FULL_WIDTH} min={0.01} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="版本" name="version"><Input /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
            <Form.Item label="生效日期" name="effective_date" {...dateFormItemProps}><DatePicker style={FULL_WIDTH} /></Form.Item>
          </Col>
          <Col xs={24} sm={24} md={12}>
      ...
      <EditableItemTable resource="bom-items" parentResource="bom-headers" parentId={record?.id} parentFk="bom_header_id" items={record?.items ?? []} columns={itemColumns} title="BOM明细" />
    </Edit>
  );
};
