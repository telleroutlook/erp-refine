...
export const SalesReturnEdit: React.FC = () => {
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title={pt('sales_returns', 'edit')}>
      ...
      <EditableItemTable resource="sales-return-items" parentResource="sales-returns" parentId={record?.id} parentFk="sales_return_id" items={record?.items ?? []} columns={itemColumns} title={t('sections.returnLines')} productsMap={productsMap} priceField="sale_price" />
    </Edit>
  );
};
